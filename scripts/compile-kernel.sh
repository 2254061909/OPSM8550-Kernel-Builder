...
#   6. FAST CHECK: prepare + build only the KernelSU driver (~1-2 min)
#   7. Build Image
#   8. Assert the features are present in the built vmlinux (symbol level)
#   9. Patch Image with the KPM loader (KPM variants only)
#
...
# =============================================================================
# FAST CHECK -- compile only the KernelSU driver.
#
# `make <subdir>/` does NOT run the prepare stage, so generated headers do not
# exist yet and any source including <generated/compile.h>, <generated/utsrelease.h>
# or the autogenerated asm offsets fails with "file not found". That is an
# artifact of the shortcut, not a real incompatibility -- SukiSU's ksu.c includes
# generated/compile.h. Run `make prepare` (and modules_prepare) first.
#
# This whole stage is an optimisation, so it is best-effort: if prepare itself
# cannot run for some reason we skip ahead to the full build rather than
...
if [[ "$IS_KSU_BUILD" -eq 1 ]]; then
  ...
  if [[ -n "$KSU_DRIVER_DIR" ]] && [[ -d "${KSU_DRIVER_DIR}/kernelsu" ]]; then
    echo "==== FAST CHECK: preparing generated headers ===="
    if make -j"$(nproc)" O=out prepare modules_prepare >/dev/null 2>&1 \
       || make -j"$(nproc)" O=out prepare >/dev/null 2>&1; then
      echo "[+] Generated headers ready."
      ...
      echo "[!] 'make prepare' did not succeed; skipping the fast check and going"
      echo "    straight to the full build (which runs prepare itself)."
    fi
  fi
fi
