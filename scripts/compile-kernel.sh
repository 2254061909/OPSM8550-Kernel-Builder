...
# =============================================================================
# FAST CHECK -- compile only the KernelSU driver (~1 min). Every C-level
# incompatibility surfaces here instead of 8 minutes into the full build.
# =============================================================================
if [[ "$IS_KSU_BUILD" -eq 1 ]]; then
...
#
# Patterns are deliberately broad. Exact upstream symbol names drift between
# releases, and a guessed name that no longer exists would fail the build after
# the most expensive step for no real reason. Presence of the subsystem is what
# we actually care about; the config assertions above pin the specifics.
# =============================================================================
...
# reports "KernelPatch was not found". Done in place so AnyKernel3 packaging,
# the release upload and the artifact upload all pick up the patched Image.
# =============================================================================
if [[ "$IS_KPM_BUILD" -eq 1 ]]; then
...
if [[ "$IS_SUSFS_BUILD" -eq 1 ]]; then
  ...
  # The config and symbol assertions above are the real gate.
